<template>
    <div class="container">
        <h1 class="mb-4 text-4xl font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-6xl"> Categories</h1>
        <div class="flex flex-wrap gap-5">
            <div v-for="cat in recipes.categories">
             <button type="button" @click="goToPage(cat.id)" class="bg-orange-300 rounded-lg w-fit max-h-fit p-2">{{ cat.name }}</button>
        </div>
        </div>
