<template>
  <div
    class="flex flex-row gap-5 p-3 border-2 rounded-lg border-orange-500 w-full items-center my-3"
    v-for="ing in ingredientsList"
  >
    <div class="flex-auto w-1/6">Ingredient {{ ing.position }}</div>
    ...
    <div class="flex justify-center items-center h-full">
      <button
        ...
        @click="deleteingredient(ing.position)"
      >
        X
      </button>
    </div>
  ...
  <div class="flex flex-row my-5 gap-2 items-center">
    ...
    <div class="flex-auto">
      <button
        type="button"
        @click="addingredient"
        class="rounded-lg p-2 bg-orange-400"
      >
        Add
      </button>
    </div>
  ...
  methods: {
    ...
    addingredient () {
      this.ingredientsList.push({
        ...
        position: this.ingredientId++,
        group: this.ingredientGroupId
      })
      this.ingredient = ''
      const inglength = this.ingredientsList.length
