<template>
    <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-200">
        <img v-if="imagesrc" class="min-h-96 min-w-96  h-96 w-96 max-w-96 max-h-96 rounded-t-lg " :src="getimagesrc" alt="Recipe Image">
        <img v-else class=" rounded-t-lg" src="../assets/mealicon.jpg" alt="Default Recipe Image">
    <div class="p-5 flex flex-col">
         <h5 class="mb-2 text-2xl font-bold tracking-tight text-orange-600  text-center">{{title}} </h5>
        <p @click="$router.push('/categories/'+categoryId)" class="text-center text-orange-300 font-extrabold hover:bg-orange-500">  {{ getCategoryNameById(categoryId).name}} </p>
        <button tyoe="button" @click="pushToPage(id)" class="inline-flex justify-center items-center px-3 py-2 text-sm font-medium text-center text-white bg-orange-300 rounded-lg hover:bg-orange-800 focus:ring-4 focus:outline-none focus:ring-orange-300  ">
            Read more
